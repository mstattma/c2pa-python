...
| Refresh `preliminary_claim` between DA invocations | `store.rs` | After each `pc.replace_assertion_by_instance(...)` call inside the DA loop, rebuild `preliminary_claim` from the now-updated provenance claim's `assertions()`. Required by CAWG Identity Assertion 1.1 §5.1.1 + §1.4 Example 3 (nested identity assertions): a DA registered later in the list can now see the *real* post-replacement hash of an earlier-registered DA's content, not just the placeholder. Without this, a publisher `cawg.x509.cose` identity assertion that references a sibling `cawg.identity_claims_aggregation` (ICA) assertion in its `signer_payload.referenced_assertions[]` would crypto-bind to the placeholder hash and fail verification. Covered by `test_dynamic_assertions_inter_da_hash_visibility`. |
